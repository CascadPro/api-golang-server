...
func MapErrors(err error) []error {
	...
	for _, fe := range verrs {
		...
		case "min":
			errs = append(errs, fmt.Errorf("`%s` minimum length is %s symbols", fe.Field(), fe.Param()))
		case "max":
			errs = append(errs, fmt.Errorf("`%s` maximum length is %s symbols", fe.Field(), fe.Param()))
		case "uuid4":
